...
#download
wget -O /usr/local/bin/PDirect.py https://raw.githubusercontent.com/myridwan/prem/ipuk/install/PDirect.py
chmod +x /usr/local/bin/PDirect.py

#setservicecdn
cat > /etc/systemd/system/cdn.service << END
[Unit]
Description=Python Edu Proxy Script By Whytzy
...
wget -O /usr/local/bin/ovpn.py PDirect.py https://raw.githubusercontent.com/myridwan/prem/ipuk/install/ovpn.py
...
[Unit]
Description=Python Edu Proxy Script By geo
Documentation=https://t.me/whytzy96
After=network.target nss-lookup.target
[Service]
Type=simple
